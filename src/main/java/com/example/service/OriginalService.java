...
import com.example.mapper.OriginalMapper;
...
public class OriginalService {
	...
	public void middleCategoryRegister() {
		...
		//親カテゴリ別にリストを生成
		List<String> sportsList = categoryList.stream().filter(str -> str.startsWith("Sports")).collect(Collectors.toList());
		List<String> womenList = categoryList.stream().filter(str -> str.startsWith("Women")).collect(Collectors.toList());
		List<String> beautyList = categoryList.stream().filter(str -> str.startsWith("Beauty")).collect(Collectors.toList());
		List<String> kidsList = categoryList.stream().filter(str -> str.startsWith("Kids")).collect(Collectors.toList());
		List<String> electronicsList = categoryList.stream().filter(str -> str.startsWith("Electronics")).collect(Collectors.toList());
		List<String> menList = categoryList.stream().filter(str -> str.startsWith("Men")).collect(Collectors.toList());
		List<String> vintageList = categoryList.stream().filter(str -> str.startsWith("Vintage")).collect(Collectors.toList());
		List<String> homeList = categoryList.stream().filter(str -> str.startsWith("Home")).collect(Collectors.toList());
		List<String> otherList = categoryList.stream().filter(str -> str.startsWith("Other")).collect(Collectors.toList());
		List<String> handmadeList = categoryList.stream().filter(str -> str.startsWith("Handmade")).collect(Collectors.toList());
		//データを1行ずつ取得してcategoryに格納
